...
function Login() {
    ...
    const errorMessages = {
        ...
        emailNotRecognised: 'Sorry, email is not supported in this version',
    };
    ...
    const redirectValidUser = (username) => {
        ...
        window.location.href = `https://${username + appConfig.hatCluster}/hatlogin?name=${APP_ID}&redirect=${redirectUrl}&fallback=${fallback}`;
    };
    ...
    const validateLoginDetails = () => {
        if (isHatName(username)) {
            ...
            setErrorMsg(errorMessages.emailNotRecognised);
            setHasError(true);
        } else {
            setHasError(true);
